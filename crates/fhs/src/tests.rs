...
fn test_fhs_disabled_returns_empty() {
    ...
    let result = fhs.generate_fhs_env("test-app", &["zlib".into(), "glibc".into()], "bin/testapp").unwrap();
...
fn test_fhs_generate_env_with_empty_deps() {
    let fhs = FhsCompat::new(true);
    let result = fhs.generate_fhs_env("test-app", &[], "bin/testapp").unwrap();
...
fn test_fhs_generate_build_expression() {
    let fhs = FhsCompat::new(true);
    let result = fhs.generate_build_fhs_expression("my-app", &["zlib".into(), "glibc".into()]).unwrap();
    assert!(result.contains("my-app"), "Should contain app name");
    assert!(result.contains("buildFHSUserEnv"), "Should use buildFHSUserEnv");
    assert!(result.contains("makeWrapper"), "Should use makeWrapper");
    assert!(result.contains("stdenv.mkDerivation"), "Should be a mkDerivation");
}

#[test]
fn test_fhs_generate_env_multiline_deps() {
    let fhs = FhsCompat::new(true);
    let deps = vec!["zlib".into(), "glibc".into(), "openssl".into(), "libX11".into()];
...
fn test_fhs_build_expression_includes_profile() {
    let fhs = FhsCompat::new(true);
    let result = fhs.generate_fhs_env("app", &["zlib".into()], "bin/app").unwrap();
