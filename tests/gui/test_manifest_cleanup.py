...
def manifest_file(tmp_path):
    """Provide a temporary manifest path by patching _manifest_path."""
    mf = tmp_path / "manifest.json"
    with patch.object(InstallWorker, "_manifest_path", return_value=mf):
        yield mf
...
class TestManifestLoadSave:
    ...
    def test_save_creates_parent_dirs(self, tmp_path):
        ...
        with patch.object(InstallWorker, "_manifest_path", return_value=deep_path):
...
class TestCleanupOrphanedEntries:
    ...
    def test_removes_orphaned_desktop_file(self, manifest_file, tmp_path):
        ...
        assert not desktop_file.exists(), "Orphaned .desktop file should be removed"
    ...
    def test_preserves_non_orphaned_files(self, manifest_file, tmp_path):
        ...
        assert desktop_file.exists(), "Non-orphaned .desktop file should be preserved"

    def test_removes_orphaned_icon_file(self, manifest_file, tmp_path):
        icons_dir = self._make_icons_dir(tmp_path)
        # The cleanup searches icons_dir / hicolor / {size} / apps / {name}{ext}
        hicolor_apps = icons_dir / "hicolor" / "48x48" / "apps"
        hicolor_apps.mkdir(parents=True, exist_ok=True)
        icon_file = hicolor_apps / "orphan-app.png"
        icon_file.write_bytes(b"\x89PNG fake")

        # Store just the name (no extension) — cleanup appends extensions itself
        InstallWorker._record_install(
        ...
        assert not icon_file.exists(), "Orphaned icon file should be removed"

    def test_no_files_to_remove_still_updates_manifest(self, manifest_file):
        InstallWorker._record_install(
    ...
    def test_mixed_orphaned_and_active(self, manifest_file, tmp_path):
        ...
        assert not orphan_desktop.exists(), "Orphaned file should be removed"
    ...
    def test_partial_nix_profile_matching(self, manifest_file):
        """Package whose base name matches a nix profile element should be preserved."""
        InstallWorker._record_install(
            "myapp", ["myapp.desktop"], []
        )
        ...
        with patch("subprocess.run") as mock_run:
            mock_result = MagicMock()
            mock_result.returncode = 0
            # Key format: "myapp-2.0" -> split(".")[-1] = "myapp-2" -> split("-")[0] = "myapp"
            mock_result.stdout = json.dumps(
                {
                    "elements": {
                        "myapp-2.0": {
